...
- **Dynamic Cluster Support**: OAuth URLs automatically generated based on `CLUSTER_NAME`
...
The cluster name is **critical for OAuth security** and must match your actual OpenShift cluster for authentication to succeed. The OAuth server URL is dynamically configured based on the cluster name and **strict issuer validation** is enforced.

**Security Implications:**
- **Exact Match Required**: Token issuer must exactly match `https://oauth-openshift.apps.${CLUSTER_NAME}`
- **No Bypass Mechanisms**: Mismatched issuers are immediately rejected
- **Authentication Failures**: Wrong cluster name will cause all OAuth authentication to fail
...
2. **OAuth Integration with Strict Security**
   - Uses OpenShift's OAuth server with **strict validation**
   - **No Authentication Bypass**: Mismatched issuers immediately rejected
   - **Issuer Validation**: Token issuer must exactly match configured OAuth server
